'''
So far, fiinder_3_w.py is a fully functional program that can search all the internal and external drives
 on a windows computer and return the search result accurately. It can accomplish this via searching every
 single branch and folder and subfolder. This takes approximately around 70 seconds on average.

 The purpose of fiinder_4.py is to try to eleminate the search time and bring it to under one second.

 fiinder_4_w.py did the following:
 1. removed items from the drivesFolders: the items removed were items that are OS items and are not meant for the average user to store data.
 2. creratred a write method. we wrote the full links to our hard drive in the same directory as the fiinder
 3. created a read method to read the full link back memory.
 4. The write method stores the link one link per line.
 5. the read method reads one link per line.
 6. now the candidate list for files are can be accessible by any one.


attempting to  solve:
Bug in this version: cant read but .txt files.
...
def checkemptyList(list_of_search_results00):
    return(len(list_of_search_results00)== 0)
